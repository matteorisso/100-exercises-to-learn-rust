...
impl Ticket {
    pub fn new(title: String, description: String, status: Status) -> Result<Ticket, String> {
        if title.is_empty() {
            let err: String = "Title cannot be empty".into();
            return Result::Err(err);
        }
        if title.len() > 50 {
            return Result::Err("Title cannot be longer than 50 characters".into());
        }
        if description.is_empty() {
            return Result::Err("Description cannot be empty".into());
        }
        if description.len() > 500 {
            return Result::Err("Description cannot be longer than 500 characters".into());
        }

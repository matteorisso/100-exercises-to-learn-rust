...
impl Ticket {
    pub fn new(title: String, description: String, status: String) -> Result<Self, TicketNewError> {
        ...
        // let proper_status = match Status::try_from(status) {
        //     Ok(status) => status,
        //     Err(err) => {
        //         return Err(TicketNewError::StatusIsInvalid { inner: err });
        //     }
        // };
        let proper_status = Status::try_from(status)?;
